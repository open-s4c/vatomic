...
data CompareCondtion = Cb_Z | Cb_NZ deriving Show
...
data Instr = Ld Bool Register Register
            ...
            | Cb CompareCondtion Register String Char
...
awaitInvariants atm = let evReg = if hasWFE atm then "assert(event_register[step] || global_monitor[step] is exclusive);\n\n"
                                  else ""
