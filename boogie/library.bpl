...
var global_monitor: [StateIndex] Monitor;
...
procedure execute(instr: Instruction)
    modifies states, effects, ordering, step, global_monitor, event_register;
        ...
        states == old(states[
            step + 1 := (
                ...
                var stx_success, cas_success :=
                    ...
                    && global_monitor[step] is exclusive
                    && global_monitor[step]->addr == gpr[instr->addr],
    ...
    ensures {:msg "global monitor"}
        (
            global_monitor == old(global_monitor[step + 1 := open()])  // extrenal write can clear monitor at any moment
            && event_register == old(event_register[step + 1 := true]) // it has to set event register
        )
        || global_monitor == old(global_monitor[step + 1 :=
            if instr is ldx then
                exclusive(states[step]->gpr[instr->addr], step)
            else
    ...
    ensures {:msg "memory"}
        old(global_monitor[step] is exclusive ==>
            memory[step, global_monitor[step]->addr] == memory[step - 1, global_monitor[step]->addr]
        );
    ...
    */
    ...
    requires {:msg "either event register or global monitor is set for WFE"}
        instr is wfe ==>
            event_register[step] || global_monitor[step] is exclusive;
        ...
        );
{
    ...
    global_monitor[step + 1] := global_monitor[step];
    ...
    ordering[step] := ConstArray(false);
        ...
        if (instr is ldx) {
            ...
            global_monitor[step + 1] := exclusive(states[step]->gpr[instr->addr], step);
        }
        if (instr is ld 
            || instr is ldx 
            || instr is swp 
    ...
    if (instr is st || 
        ...
        instr is ldadd || instr is stadd) {
        stx_succeeds :=
            states[step]->local_monitor is exclusive &&
            states[step]->local_monitor->addr == states[step]->gpr[instr->addr] &&
            global_monitor[step] is exclusive &&
            global_monitor[step]->addr == states[step]->gpr[instr->addr];
        // stx always clears local monitor
        if (instr is stx || store_clears_local_monitor) {
            states[step + 1]->local_monitor := open();
        }
        if (store_clears_global_monitor) {
            global_monitor[step + 1] := open();
        }
        if (instr is stx) {
            if (stx_succeeds) {
    ...
    if (global_monitor[step] is exclusive) {
        assume(memory[step, global_monitor[step]->addr] == memory[step - 1, global_monitor[step]->addr]);
    }
