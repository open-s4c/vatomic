...
std::vector<MAP_T_s16> g_values = {0, MAP_VALS_s16};

void
assert_match(vsync::atomic<MAP_T_s16> &var, std::atomic<MAP_T_s16> &mirror)
{
    MAP_T_s16 v_var    = var.load();
    MAP_T_s16 v_mirror = mirror.load();
    if (v_var != v_mirror) {
...
test_init(void)
{
    for (MAP_T_s16 val : g_values) {
        std::atomic<MAP_T_s16> mirror(val);
        vsync::atomic<MAP_T_s16> var(val);
...
test_store(void)
{
    std::atomic<MAP_T_s16> mirror(0);
    vsync::atomic<MAP_T_s16> var(0);
