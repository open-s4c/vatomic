...
std::vector<vint16_t> g_values = {0, VINT8_MAX, VINT16_MAX};

void
assert_match(vsync::atomic<vint16_t> &var, std::atomic<vint16_t> &mirror)
{
    vint16_t v_var    = var.load();
    vint16_t v_mirror = mirror.load();
    if (v_var != v_mirror) {
...
test_init(void)
{
    for (vint16_t val : g_values) {
        std::atomic<vint16_t> mirror(val);
        vsync::atomic<vint16_t> var(val);
...
test_store(void)
{
    std::atomic<vint16_t> mirror(0);
    vsync::atomic<vint16_t> var(0);
