...
std::vector<MAP_T_s32> g_values = {0, MAP_VALS_s32};

void
assert_match(vsync::atomic<MAP_T_s32> &var, std::atomic<MAP_T_s32> &mirror)
{
    MAP_T_s32 v_var    = var.load();
    MAP_T_s32 v_mirror = mirror.load();
    if (v_var != v_mirror) {
...
test_init(void)
{
    for (MAP_T_s32 val : g_values) {
        std::atomic<MAP_T_s32> mirror(val);
        vsync::atomic<MAP_T_s32> var(val);
...
test_store(void)
{
    std::atomic<MAP_T_s32> mirror(0);
    vsync::atomic<MAP_T_s32> var(0);
