...
std::vector<vint32_t> g_values = {0, VINT8_MAX, VINT16_MAX, VINT32_MAX};

void
assert_match(vsync::atomic<vint32_t> &var, std::atomic<vint32_t> &mirror)
{
    vint32_t v_var    = var.load();
    vint32_t v_mirror = mirror.load();
    if (v_var != v_mirror) {
...
test_init(void)
{
    for (vint32_t val : g_values) {
        std::atomic<vint32_t> mirror(val);
        vsync::atomic<vint32_t> var(val);
...
test_store(void)
{
    std::atomic<vint32_t> mirror(0);
    vsync::atomic<vint32_t> var(0);
