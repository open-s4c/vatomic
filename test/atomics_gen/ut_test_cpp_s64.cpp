...
std::vector<vint64_t> g_values = {0, VINT8_MAX, VINT16_MAX, VINT32_MAX,
                                  VINT64_MAX};

void
assert_match(vsync::atomic<vint64_t> &var, std::atomic<vint64_t> &mirror)
{
    vint64_t v_var    = var.load();
    vint64_t v_mirror = mirror.load();
    if (v_var != v_mirror) {
...
test_init(void)
{
    for (vint64_t val : g_values) {
        std::atomic<vint64_t> mirror(val);
        vsync::atomic<vint64_t> var(val);
...
test_store(void)
{
    std::atomic<vint64_t> mirror(0);
    vsync::atomic<vint64_t> var(0);
