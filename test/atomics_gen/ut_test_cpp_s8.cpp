...
{
    vint8_t v_var    = var.load();
    vint8_t v_mirror = mirror.load();
    if (v_var != v_mirror) {
...
test_init(void)
{
    for (vint8_t val : g_values) {
        std::atomic<vint8_t> mirror(val);
        vsync::atomic<vint8_t> var(val);
...
test_store(void)
{
    std::atomic<vint8_t> mirror(0);
    vsync::atomic<vint8_t> var(0);
